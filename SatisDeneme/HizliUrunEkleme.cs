...
{
    public partial class HizliUrunEkleme : Form
    {
        public HizliUrunEkleme()
        {
            InitializeComponent();
        }
        ...
        private void tUrunAra_TextChanged(object sender, EventArgs e)
        {
            if (tUrunAra.Text != "")
            {
                ...
                var urunler = db.Urun.Where(a => a.UrunAd.Contains(urunad)).ToList();//contains karaktere göre listeleme yapıyor
        ...
        private void gridUrunler_CellContentDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (gridUrunler.Rows.Count > 0)
            {
                ...
                Form1 f = (Form1)Application.OpenForms["fSatis"];
                if (f != null)
                {
